...
def test_displacements_meoh(tmpdir):
    ...
    freqs, normal_modes = harmonic_vibrational_analysis(
        hessian, meoh, proj_translations=True, proj_rotations=True, ascomplex=False
    )

    with open(os.path.join(cwd, "data", "meoh_images.pkl"), "rb") as fpkl:
        refimages = pickle.load(fpkl)

    images, modeinfo = calculate_displacements(
        meoh, hessian, freqs, normal_modes, npoints=4
    )
    ...
    for mode in images.keys():
        for point in images[mode].keys():
            assert np.allclose(
                images[mode][point].positions, refimages[mode][point].positions
            )
